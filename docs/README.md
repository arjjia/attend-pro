...
- вторичная зашифрованная local-first реплика доказательства в Giper Baza;
...
flowchart LR
    ...
    G["Giper Baza private Land<br/>вторичная реплика"]
...
- Giper Baza даёт дополнительный локальный signed/encrypted журнал;
...
2. Нажмите «Запустить тестовую пару сейчас». Backend сдвинет окно пары на
   `now - 5 минут … now + 85 минут`, отзовёт старое разрешение и выдаст новое.
3. Нажмите «Создать QR “Вход”». Браузер преподавателя создаст nonce, подпишет
   challenge своим приватным ключом и покажет QR на 90 секунд.
...
заглушкой. Для устройства создаётся private Land с запрещённым anonymous-доступом;
полное каноническое доказательство добавляется в signed/encrypted CRDT-журнал.
В основной proof включается ссылка вида `giper://<land>/<claim>`.

Но Giper намеренно не является единственной официальной БД:

- доступность secondary replica не блокирует создание отметки;
...
- в MVP нет развёрнутого общего Giper relay и обмена правами между устройствами.

Следующий инфраструктурный этап — университетский Giper relay, политика ACL и
репликация private Land на второе доверенное устройство. Wire-протокол
посещаемости для этого менять не нужно.
...
- Giper relay, backup/ACL между несколькими устройствами и наблюдаемость репликации;
